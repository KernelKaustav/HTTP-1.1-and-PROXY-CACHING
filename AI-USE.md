...
| 20 Sep | Puskar Das | Used Claude. Explained the Selector/non-blocking-channel model (java.nio) and generated an initial draft of `EventLoopServer.java`'s skeleton (Selector setup, ServerSocketChannel registration for OP_ACCEPT, the select() loop) | Rewrote in my own style, renamed variables, restructured comments after understanding the selector-key lifecycle and why keys must be removed manually. OP_ACCEPT/OP_READ/OP_WRITE handling deliberately stubbed for later commits (24 Sep, 28 Sep, 02 Oct) per schedule. |
## Notes for teammates
